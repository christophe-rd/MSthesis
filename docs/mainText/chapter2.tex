...
With this model, we estimated a species-specific effect of each season predictor on growth ($\beta_{species}$), but we also controlled for variation due to species ($\alpha_{species}$), individual tree ($\alpha_{treeid}$) and year ($\alpha_{year}$). We first ran the models using natural units and then scaled the predictors through $z$-scoring so we could directly compare their effect size \citep{gelman_data_2006}. We report estimates as means and 90\% uncertainty intervals (UI). We also fit the models for SOS and EOS using the full vs restricted datasets. Finally, because the SOS diverge across species with different wood anatomy, we ran the models using budburst instead of leafout as the SOS---allowing us to test whether our model responses diverged across diffuse vs ring porous species (Table \ref{tab:tsCount}).\\
...
($\beta =$ 0.15, UI:
0.02,
0.28 \RW scaled GDD), and the current year GDD decreased its growth, like the observed response when the previous year conditions were not considered. {\Ball}'s congeneric {\Bnig}, also grew less with previous year GDD ($\beta =$ 
-0.08, UI:
-0.17,
-0.00 \RW scaled GDD; Figure \ref{fig:tsPrvsYr}b; Table \ref{tab:tab_PrvsYr}) but in contrast, current year GDD increased growth for that species ($\beta =$ 
0.12, UI:
0.03,
...
1.04, UI:
0.39,
1.70 $\Delta$ EOS/SOS; Figure \ref{fig:SOSonEOS}. We did not find any clear difference on baseline growth across years (Figure \ref{fig:tsmuplotayear}; Table \ref{tab:tab_all}) and between species (Figure \ref{fig:tsxyGDDyr}; \ref{fig:tsxyGSLyr}). Growth was similar across individual trees (Figure \ref{fig:tsmuplotatreeid}).
...
\hline
 ...
 American basswood (\textit{Tilia americana}) & Fast & Diffuse-porous &   5 \\ 
  ...
  Yellow birch (\textit{Betula alleghaniensis}) & Slow & Diffuse-porous &   4 \\ 
...
\begin{table}[ht]
\centering
\caption{Summary output of each parameter from our four predictors, using ring width (log(mm)) as the reponse variable (See equation \ref{eq:mainMdl}). We report estimates as mean and 90\% uncertainty intervals (in square brackets) across our four different predictors (GDD: growing degree days, GSL: growing season length, SOS: start of season and EOS: end of season). See `Data analyses' section in Methods for more details.} 
\label{tab:tab_all}
\begin{tabular}{lllll}
  \hline
...
\begin{table}[ht]
\centering
\caption{Summary output of each parameter from the previous year model (Equation \ref{eq:prvsYrMdl}) using ring width (log(mm)) as the reponse variable. We report estimates as mean and 90\% uncertainty intervals (in square brackets). see `Data analyses' section in Methods for more details} 
