...
% Paragraph 3
New studies suggest that longer GS do not always increase growth \citep{dow_warm_2022,silvestro_longer_2023,matula_shifts_2023,charletdesauvage_temperature_2022,cabon_crossbiome_2022}. Recently, \cite{dow_warm_2022} found that warmer springs lengthened the growing season, but did not increase the peak growth rate or total annual growth in two temperate forest communities. Working across biomes, \cite{cabon_crossbiome_2022} found a similar trend, where gross primary production and growth appeared decoupled. However, neither of these studies could clearly establish why they found no relationship while previous studies have. Understanding whether temperate forests will persist as a net carbon sink requires answering why trees do not grow more despite longer GS. 
...
(22.3833$^\circ$C), 2020 the hottest 
(23.0667$^\circ$C), and 2018 in between 
...
0.3895 mm ({\Ball} in 2020) to 
13.5467 mm ({\Bpop} in 2018; Figure \ref{fig:wcboxplotRW}).  
...
0.148, UI:
0.062, 
0.231 RW/GDD), Grey birch (\textit{Betula populifolia}; $\beta =$
0.069, UI: 
0.023, 
0.116 RW/GDD), and Yellow birch (\textit{Betula alleghaniensis}; $\beta =$
0.04, UI: 
0.00, 
0.08 RW/GDD), grey alder (\textit{Alnus incana}) trended in the same direction
($\beta =$ 0.021, UI: 
-0.011, 
0.054 RW/GDD); Figure \ref{fig:wcmuplotbspp}a and e; Table\ref{tab:wc_slope_table}).
...
($\beta =$ 0.181, UI: 
0.049, 
0.308 RW/day), while the other two \textit{Betula} did not, though they trended in the same way ({\Ball}: $\beta =$
0.03, UI: 
-0.03, 
0.09 RW/day, and {\Bpop}: $\beta =$
0.054, UI: 
-0.021, 
0.129 RW/day; Figure \ref{fig:wcmuplotbspp}b and f; Table \ref{tab:wc_slope_table}). 
...
-0.073, UI:
-0.133, 
-0.012 RW/day; Figure \ref{fig:wcmuplotbspp}c and g; Table \ref{tab:wc_slope_table}). 
...
0.115, UI:
0.051, 
0.178 RW/day). Alongside {\Ball}, two other species grew more with a later end of season ({\Bpop}: $\beta =$
0.10, UI:
0.02, 
...
-0.664, UI:
-4.943, 
3.635 RW; Figure \ref{fig:wcxyGDD} and \ref{fig:wcxyGSL}; Table \ref{tab:tab_all}). In addition, growth did not vary with provenance (Dartmouth College (NH): $\alpha =$
-0.028, UI:
-0.22, 
0.143 RW; Harvard Forest (MA): $\alpha =$
-0.10, UI:
-0.328, 
0.066 RW; St-Hippolyte (Qc): $\alpha =$
0.04, UI:
-0.14, 
0.244 RW; White Mountains (NH): $\alpha =$
0.078, UI:
-0.091, 
0.29 RW; Figure \ref{fig:wcmuplotasite}; Table \ref{tab:wc_asite_table}). The limited variation between provenances ($\sigma = $
0.171, UI:
0.017, 
0.475 RW) was similar to variation across individual trees ($\sigma =$ 
0.17, UI: 
0.048, 
0.268 RW; Table \ref{tab:tab_all}).
