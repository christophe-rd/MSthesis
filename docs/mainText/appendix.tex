...
\begin{table}[ht]
\centering
\caption{Provenance coordinates and 1951--1980 climate normals: mean annual temperature (MAT; \degree C), mean summer precipitation (MSP; mm), and frost-free period (FFP; days). We extracted the climate variables at each provenance from \citep{wang_locally_2016}} 
\label{tab:climate}
...
\caption{The effect of provenance on growth across a latitudinal gradient. We show the ring width (log(mm); see Table \ref{tab:tab_all} for all parameter estimates) for each provenance with growing degree days as the predictor (effects were similar for the other predictors; see Table \ref{tab:wc_asite_table}). The dots represent the estimated mean of each provenance, with 50\% and 90\% uncertainty intervals (thicker and thinner lines, respectively). The provenances are color-coded from Figure \ref{fig:mapProv}, the map showing their locations. We also performed a separate analyses, fitting the provenances per species, individually (see Figure \ref{fig:wcmuplotasitePerSpp})}
\label{fig:wcmuplotasite}
\end{figure}

% Wildchrokie provenances mu plot per species
\begin{figure}[!ht]
\includegraphics[width=0.8\textwidth]{../figures/wildchrokie/growthModelsMain/muProvPerSpp.pdf}
